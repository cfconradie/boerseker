  ...
  template: __BOERSEKER_TPL__,
        setup() {
          ...
          function scrollCalcToTop() {
            // Two reasons to defer + run twice:
            //  1. Vue must re-render the new wizard step before we measure.
            //  2. CloudFunnels/Simvoly sometimes runs its own scroll handler
            //     (sticky header / analytics) that fights smooth-scroll mid-flight.
            //  Using behavior:'auto' (instant) survives that, then a second pass
            //  after one frame catches any layout shift.
            nextTick(() => {
              const el = document.getElementById('boerseker-app');
              if (!el) return;
              const doScroll = () => {
